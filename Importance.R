...
importance_quantile <- function(form, data, qt, rep = 10){
  ...
  for (q in 1:length(qt)){
    pinball.base[q] <- mean(pinball_loss(target = data[,target],
                                         pred = predict(rf.base, data = data, type = "quantiles", quantiles = qt[q])$predictions,
                                         tau = qt[q]))
  }
  for (v in variables){
    for (r in rep){
      ...
      for (q in 1:length(qt)){
        pinball.rep <- mean(pinball_loss(target = data[,target],
                                         pred = predict(rf.base, data = data_rep, type = "quantiles", quantiles = qt[q])$predictions,
                                         tau = qt[q]))
...
colnames(importance)[5:11] <-  paste("GAM_", c("Load.336", "WeekDays:DLS", "WeekDays:Load.48", "Date:Temp",
                                               "Temp_s95", "Temp_s99", "Temp_s99_min:Temp_s99_max"), sep = '')
colnames(importance)[12:17] <- substr(colnames(importance)[12:17],1,nchar(colnames(importance)[12:17])-4)
colnames(importance)[2:3] <- c("residuals.48", "residuals.336")
